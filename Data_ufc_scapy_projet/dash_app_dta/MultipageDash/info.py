...
#df2 = pd.read_json(r"ufc_database.ufc_fight (1).json")

#df = pd.read_json(r"ufc_database.ufc_fighters.json")



# Fonction pour charger les données si nécessaire
def load_data_if_empty(collection, json_file_path):
    if collection.count_documents({}) == 0:  # Vérifie si la collection est vide
        with open(json_file_path, 'r') as file:
            data = json.load(file)
        df = pd.DataFrame(data)
    else:
        data = list(collection.find({}, {'_id': 0}))  # Exclure l'ID MongoDB dans le DataFrame
        df = pd.DataFrame(data)
    return df
...
fighter_json_path = 'ufc_database.ufc_fighters.json'
fights_json_path = 'ufc_database.ufc_fight_1.json'
...
data2 = load_data_if_empty(collection_fights, fights_json_path)
...
data = load_data_if_empty(collection, fighter_json_path)
df = pd.DataFrame(data)
...
df_combined_stk_body['SIG_STK_BODY'] = pd.to_numeric(df_combined_stk_body['SIG_STK_BODY'], errors='coerce')
...
df_combined_sig_pos_ground['SIG_POS_GROUND'] = pd.to_numeric(df_combined_sig_pos_ground['SIG_POS_GROUND'], errors='coerce')
